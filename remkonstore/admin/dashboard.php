...
// Calculate today's profit metrics
$result = $conn->query("
    SELECT 
        SUM(si.quantity * p.price) AS revenue,
        SUM(si.quantity * po.rate) AS cost
    FROM sale_items si
    JOIN sales s ON si.sale_id = s.id
    JOIN products p ON si.product_id = p.id
    JOIN product_orders po ON si.product_id = po.product_id
        AND DATE(s.sale_date) = po.order_date
    WHERE DATE(s.sale_date) = '$today'
");

if ($result && $row = $result->fetch_assoc()) {
    $profitMetrics['today']['revenue'] = $row['revenue'] ?? 0;
    $profitMetrics['today']['cost'] = $row['cost'] ?? 0;
    $profitMetrics['today']['profit'] = $profitMetrics['today']['revenue'] - $profitMetrics['today']['cost'];
}
...
$result = $conn->query("
    ...
    JOIN product_orders po ON si.product_id = po.product_id
        AND DATE(s.sale_date) = po.order_date
    WHERE s.sale_date BETWEEN '$lastWeek' AND '$today'
");

if ($result && $row = $result->fetch_assoc()) {
    $profitMetrics['week']['revenue'] = $row['revenue'] ?? 0;
    $profitMetrics['week']['cost'] = $row['cost'] ?? 0;
    $profitMetrics['week']['profit'] = $profitMetrics['week']['revenue'] - $profitMetrics['week']['cost'];
}
...
$result = $conn->query("
    ...
    JOIN product_orders po ON si.product_id = po.product_id
        AND DATE(s.sale_date) = po.order_date
    WHERE s.sale_date BETWEEN '$lastMonth' AND '$today'
");

if ($result && $row = $result->fetch_assoc()) {
    $profitMetrics['month']['revenue'] = $row['revenue'] ?? 0;
    $profitMetrics['month']['cost'] = $row['cost'] ?? 0;
    $profitMetrics['month']['profit'] = $profitMetrics['month']['revenue'] - $profitMetrics['month']['cost'];
}
