...
if ($checkAdmin->num_rows === 0) {
    ...
    $conn->query("INSERT INTO users (username, password, role) VALUES ('admin', '$hashed_password', 'admin')");
}

// Add default cashier user if not exists
$checkCashier = $conn->query("SELECT * FROM users WHERE username = 'cashier'");
if ($checkCashier->num_rows === 0) {
    $hashed_password = password_hash('cashier123', PASSWORD_DEFAULT);
    $conn->query("INSERT INTO users (username, password, role) VALUES ('cashier', '$hashed_password', 'cashier')");
}
